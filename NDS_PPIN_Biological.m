function [individual, Front, fv1] = NDS_PPIN_Biological(Pop_Size,Index_Of_Chromosome,adj,shortest_path_matrix,similarity_matrix)
...
for i=1:Pop_Size
    ...
    individual(i).Sp=[];
    individual(i).np=0;
    fv1(i,:)=[f1_graph(Index_Of_Chromosome{i},adj,shortest_path_matrix),f2_Similarity(Index_Of_Chromosome{i},similarity_matrix)];%目标函数
    for j=1:Pop_Size
%         x1=dec(pop0(i,:));
%         x2=dec(pop0(j,:));
...
%         t= Check_Dominance_PPIN_Biological(fv1(i,:),fv2);     %individual(i) dominate individual(j) Check_Dominance(individual(i))
        if Check_Dominance_PPIN_Biological(fv1(i,:),fv2)  %检查Pareto占优，如果i解占优j解
        individual(i).Sp=[individual(i).Sp,j];%被i占优的解集
        else if  Check_Dominance_PPIN_Biological(fv2,fv1(i,:))         % ind(i)is dominated by ind(j)check_Dominance(%(obj1(x2)<obj1(x1) & obj2(x2)<=obj2(x1)) | (obj1(x2)<=obj1(x1) & obj2(x2)<obj2(x1))
